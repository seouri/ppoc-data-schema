...
The truth manifest is written only when an evaluator explicitly supplies a new regular non-symlink destination. Ancestors are opened and pinned component-by-component with no-follow directory descriptors; the child is created directly with exclusive no-follow mode `0600`, then kept open for descriptor-pinned readback, identity verification, size bounding, fsync, and canonical reparse without overwriting an existing path. Failed writes or verification detach the child with an atomic no-replace rename into a private descriptor-pinned cleanup namespace, compare it with the still-open owner descriptor, and never unlink a quarantined inode. A same-name unlink/recreate replacement is restored with a no-overwrite hard link when possible, while the owner or replacement remains in private quarantine and cleanup reports an explicit error; non-linkable replacements remain private and are reported the same way. The namespace is not removed by pathname after the descriptor closes, so a pathname swap cannot cause a different directory to be deleted. Utilization-intensity and measurement-error-removal interventions are rejected until the observation/resource layer exists. This trajectory contract does not generate complete paired eight-resource packages and is not prevalence, demographic, clinical, temporal-drift, task-utility, privacy, non-matchability, Synthea, or release evidence; package-level counterfactual generation remains a separate roadmap gate. See [the counterfactual usage section](docs/synthetic-generator.md#evaluator-only-trajectory-counterfactual-validation) for the API and evaluator boundary.
