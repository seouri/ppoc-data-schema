...
The native generator remains the release-one route and is also the ordinary development route. An optional external Synthea development adapter is implemented separately; use the [Synthea backend guide](synthea-backend.md) for its pinned checkout, Java 17/Gradle 9.2.1 prerequisites, one command, exact-schema projection, and healthy/GHD growth boundary. The [optional Synthea engine-conformance guide](synthea-conformance.md) remains a separate declaration and evidence contract: the adapter does not establish conformance, clinical validity, representative prevalence, privacy/non-matchability, or release authority. The adapter is not imported automatically by generation, export, or evaluator code (or by the native profile selector) and does not change the production command, which remains fail closed with `No production growth reference or authoritative derivation oracle is configured`.
...
`development-all-disorders` is the fourth explicit route, versioned as `development-all-disorders-v1`, for broad fictional trajectory and integration coverage. Run the exact command above. It keeps the snapshot-shaped demographics from `schema/stats.json` snapshot `2026-08-24`, but subtype labels are unavailable in the observed marginals, so its F/M conditional prior is a fictional coverage scenario rather than a prevalence estimate:

| Reference sex | Healthy | Each compatible nonhealthy module | Turner syndrome |
| --- | ---: | ---: | ---: |
| F | `1/2` | `1/18` for each of the nine nonhealthy kinds | `1/18` |
| M | `1/2` | `1/16` for each of the eight non-Turner kinds | `0` |

Turner is F-reference-only; the U sampling weight remains zero because the pinned CDC reference has no U series. The seven reviewed ancillary dispatch targets are growth-hormone deficiency (GHD), pediatric hypothyroidism, celiac disease, small-for-gestational-age (SGA), Turner syndrome, undernutrition, and excess weight. Only matching reviewed projections create ancillary rows; healthy, familial short stature, and constitutional delay remain empty. GHD alone receives the fixed synthetic `E23.0` diagnosis token. All four routes use the exact eight-resource exporter and manifest boundary, serialize the in-memory fictional `Synthetic` lab marker as `result_flag=""`, and keep hidden truth, latent module state, and patient diagnostics out of visible files.

All four profiles load the repository `datapackage.json` by default and default `--reference-time` to `2026-09-01T00:00:00Z` and `--software-revision` to `development-generator-v1`; callers may override those metadata values for reproducibility experiments. They use the pinned `cdc-lms-reference-v1` CDC LMS adapter and the `development-augmenter-v1` source-matched binding. Before generation, the runtime verifies the manifest-listed 14-file augmenter closure and the pinned `uv.lock` dependency bytes. The source-matched growth augmenter is not bound as authoritative for clinical, prevalence, privacy, or release decisions. A successful manifest identifies the profile, schema/reference/derivation fingerprints, configuration hash, and `test_only_derivation=true`.
...
## Optional external Synthea backend

The Synthea-dependent route is implemented as an explicit external adapter rather than as another native profile. It runs only a caller-supplied checkout at revision `d9d07a6eef91ee5144293b42ab64224d84d124f8`, requires Java 17 and its Gradle 9.2.1 wrapper, and uses the versioned fictional GHD module plus the deterministic `synthea-growth-overlay-v1` adapter. Synthea's pediatric FHIR measurements form the healthy trajectories; GHD members receive the fixed development prior and bounded height/weight/BMI overlay. The adapter projects exact-schema rows, delegates augmentation to the test-only source-matched runtime, and writes no raw FHIR or hidden state. It is not required for ordinary synthetic development.

Use the [Synthea backend guide](synthea-backend.md) for the one command, cache/toolchain setup, output inventory, manifest fields, reproducibility check, and troubleshooting. The backend's `ghd_count` is a fictional scenario count, not validated prevalence. Java 17 remains required even if a newer local Gradle compiles with Java 26: the pinned wrapper fails with Java 26, and the separate upstream full-test exit `134` occurs under both Java 26 and Java 17. Neither result is Synthea conformance or privacy/non-matchability evidence.

## Ordinary development requirements

The four explicit profiles are self-contained synthetic-development workflows. They require only a repository checkout, the pinned public/reference runtime already in the checkout, `uv sync`, a new output path, a positive patient count, and a seed. The descriptor supplies schema shape; the versioned development configuration supplies the fictional demographic weights, disorder prior, observation policy, and age schedule; and the pinned CDC reference plus source-matched augmenter supplies reproducible growth and derived fields. No real-data root, IRB/DUA authorization, partition key, disclosure or privacy policy, calibration/held-out artifact, release decision, Synthea checkout, model, or patient-level input is needed.
...
The scheduled development scale profile collects seven opt-in, test-only logical checks with the prescribed command: three native direct cohort checks at seeds `20260830`, `20260831`, and `20260901`; one native `development-all-disorders` composition check covering all ten module kinds; and three public CLI package-composition checks for `development-cohort`, `development-realistic`, and `development-all-disorders`. The native checks exercise the in-memory cohort, cohort/temporal/task evaluators, exact-schema exporter, and source-matched augmenter oracle; the CLI checks exercise the pinned CDC reference and source-matched derivation runtime. Run all seven with:
...
The native direct parameterized checks use the fixed seeds `20260830`, `20260831`, and `20260901`, generate an exact 10,000-patient fictional cohort per seed, and write each temporary package beneath pytest's `tmp_path` rather than retaining output in the repository. They check all eight descriptor resources, exact row counts and schema identity, test-only derivation, longitudinal drift, and visible-only task execution. The native all-disorder check uses the fixed full-age schedule and confirms all ten module kinds occur while Turner remains F-reference-only. The `development-cohort` CLI composition check uses seed `20260901`, confirms every generated patient has the fixed full-age schedule, and checks the public package inventory, unique synthetic patient/visit IDs, manifest identity fields, and absence of latent truth in every artifact. The `development-realistic` CLI composition check uses the same scale and seed, verifies the target-shaped GHD ancillary relationships (`labs = 2 * problem_list = 2 * referrals` and `growth_dx_flag` equals the problem-list count), confirms conditional medication bounds, and checks the typed GHD lab marker's serialization sentinel is the exact descriptor missing-value sentinel. The `development-all-disorders` CLI composition check uses the same scale and seed, verifies the exact eight-resource package and manifest profile, and confirms all supported ancillary pathways serialize within the unchanged schema boundary. The opt-in keeps these multi-minute development checks out of ordinary CI; normal focused runs collect the tests but skip them unless `SYNTHETIC_RUN_SCALE=1` is set.
...
This API is development orchestration, not evidence of prevalence validation, demographic representativeness, held-out validation, privacy/non-matchability, clinical validity, task utility, or ancillary resources and clinical pathways beyond the GHD projection in this calibrated-cohort example. The separate all-disorder package route is likewise not authoritative derivation, release approval, or Synthea conformance. Those remain separate deferred gates; the production smoke CLI remains fail closed.
...
The profile report is evaluator-only aggregate diagnostics, not prevalence validation, demographic representativeness, held-out validation, clinical validity, privacy evidence, non-matchability proof, package or release evidence, task utility, or ancillary clinical pathways beyond the GHD projection in this report's example. The separate all-disorder package route does not turn this report into evidence or authority. It does not prove that a generated patient profile cannot be matched to a real patient; use the separately governed privacy evaluation for qualified, policy-bound evidence, which also cannot prove non-matchability. The production smoke CLI, exact-schema package exporter, held-out validator, privacy auditor, and optional Synthea route remain unchanged and separately gated.
...
This evaluator contract does not itself implement augmented resources, exact-schema file export, or package manifests. The development-only bridges below write exact-schema files and manifests with an explicit injected oracle; authoritative augmented clinical derivation, prevalence and demographic calibration, unreviewed ancillary clinical pathways beyond the reviewed GHD and all-disorder projections, held-out validation, privacy/non-matchability evaluation, task utility, clinical validity, release approval, and Synthea conformance remain explicitly deferred gates.
...
sentinel, while the generic empty-ancillary route remains unchanged. The
published `development-all-disorders` route is the second package-level route;
it uses the multidisorder adapter to compose only matching projections for the
seven reviewed ancillary targets. Complete package export for paired
counterfactual worlds, non-target profiles, and authoritative derivation
remains deferred, as do unreviewed ancillary pathways, prevalence calibration,
held-out validation, privacy and non-matchability evidence, clinical review,
task utility, release approval, and Synthea conformance. The default production
CLI remains fail-closed, and existing empty-ancillary base-resource contracts
remain in force for the evaluator-only generic route.
...
This synthetic-only, evaluator-only seam is itself in-memory only: it has no file input, package/file export, manifest, descriptor mutation, or CLI. The explicit `development-realistic` runtime is the first package-level caller that projects and merges these typed GHD rows, converts the fictional lab marker to the exact descriptor sentinel, flattens all six resources, and sends them through the existing exact-schema exporter and source-matched augmenter. The published `development-all-disorders` runtime is the second package-level caller and performs the same boundary for the matching typed projection among its seven reviewed ancillary targets. The merge remains the reviewed seam used by the paired counterfactual worlds composer below; it does not make paired counterfactual worlds, non-target profiles, or authoritative augmented derivation exportable. Those package/export paths, unreviewed ancillary pathways, prevalence or demographic calibration, held-out validation, clinical review, task utility, privacy or non-matchability, release approval, and Synthea conformance remain separately deferred.

For this evaluator-only GHD section, complete package export for other disorders remains outside this typed seam; the published all-disorder route is the reviewed package-level exception. Augmented derivation, prevalence, held-out validation, privacy, non-matchability, clinical review, task utility, release evidence, and Synthea evidence remain deferred for this direct API, and the default production path remains fail-closed.
...
The boundary accepts only a typed `CohortMember`, an already extracted `ResourceShape`, and an `ExcessWeightAncillaryPolicy`. It returns an immutable exact-schema projection with four fixed resource tuples: `labs`, `medications`, `problem_list`, and `referrals`. Every row follows the supplied descriptor field order and uses the exact empty-string sentinel for omitted fields. This evaluator-only API accepts no descriptor path, CSV reader, package writer, output destination, CLI, or governed input; the published `development-all-disorders` runtime separately consumes its matching typed projection for package export.
...
`EXCESS_WEIGHT` is evaluator-only as a latent trajectory kind; it neither implies nor writes `obesity_flag`, which remains separately derived from observed BMI percentile. Healthy, GHD, and other non-target members return four empty tuples from this direct evaluator-only API. The published `development-all-disorders` runtime may consume the matching projection as one of its seven reviewed ancillary targets; that package route remains fictional coverage, not prevalence evidence, privacy/non-matchability proof, clinical validity or treatment guidance, release authorization, or Synthea conformance. Prevalence calibration, held-out comparison, privacy review, clinical review, release authorization, and optional Synthea conformance remain deferred.
...
These labels are fictional/nonclinical and make no ICD, LOINC, or RxNorm claim. Latent state remains hidden, and output is deterministic, immutable, exact-schema, and evaluator-only; healthy and all other disorder kinds return empty tuples from this direct API. This pathway does not write or infer `obesity_flag`. The published `development-all-disorders` CLI separately composes its matching celiac projection for package-level fictional coverage. This direct evaluator-only API has no runtime/package integration or package export; those remain deferred for this API and any unreviewed pathways. Prevalence/demographic calibration, privacy/non-matchability, clinical review, release authorization, real or held-out data, and optional Synthea conformance remain deferred and are not ordinary-development prerequisites.
...
This direct SGA API has no file or CLI integration; the published `development-all-disorders` route separately composes its matching typed projection for package-level fictional coverage. Runtime/package integration remains deferred for this direct API and any unreviewed pathway. That route is not prevalence/demographic calibration, privacy/non-matchability evidence, clinical review or guidance, release authorization, real or held-out data work, or Synthea conformance. Additional resource expansion, prevalence/demographic calibration, privacy/non-matchability, clinical review, release authorization, real or held-out data, and optional Synthea conformance remain deferred and are not ordinary-development prerequisites.

The direct SGA API also keeps gestational-age resource expansion deferred for this evaluator-only boundary; the reviewed all-disorder package route is the explicit package-level exception.
...
Healthy and every non-Turner member return four empty tuples from this direct evaluator-only API, including GHD, hypothyroidism, celiac, SGA, undernutrition, excess weight, familial short stature, and constitutional delay. The published `development-all-disorders` route separately composes the matching Turner projection for female-reference members; it remains fictional coverage. Runtime/package integration remains deferred for this direct API and any unreviewed pathway. Prevalence/demographic calibration, privacy/non-matchability, clinical review, clinical/release claims, release authorization, real or held-out data, and optional Synthea conformance remain deferred in this ordinary-development fixture contract; these are not ordinary-development prerequisites.

## Evaluator-only undernutrition ancillary pathway

`project_undernutrition_ancillary_resources(member, shape, policy)` and `validate_undernutrition_ancillary_resources(member, projection, policy)` are deterministic, typed in-memory, exact-schema, evaluator-only APIs for one fictional member. Their public models are `UndernutritionAncillaryPolicy`, `UndernutritionAncillaryProjection`, `UndernutritionAncillaryProjectionUnavailable`, `UndernutritionAncillaryValidationStatus`, `UndernutritionAncillaryCheck`, and `UndernutritionAncillaryValidationReport`. The finalized public constants also include `UNDERNUTRITION_ANCILLARY_RESOURCE_NAMES`, `UNDERNUTRITION_LAB_COMPONENT_NAMES`, `UNDERNUTRITION_ANCILLARY_CHECK_NAMES`, `UNDERNUTRITION_ANCILLARY_REASON_CODES_BY_STATUS`, and `UNDERNUTRITION_ANCILLARY_REASON_CODES`.

This boundary accepts a typed `CohortMember`, a policy, and an already extracted `ResourceShape`. That `ResourceShape` is the six-resource input shape, whose specifications are fixed in this order: `patients`, `visits`, `labs`, `medications`, `problem_list`, and `referrals`. The four-resource projection output named by `UNDERNUTRITION_ANCILLARY_RESOURCE_NAMES` contains only `labs`, `medications`, `problem_list`, and `referrals`, in that order. Every output tuple preserves the supplied descriptor field order and empty-string missing-value conventions. The current exact emitted orders are:

```text
labs: patient_id, visit_id, lab_order_id, result_line_num, lab_order_date_age_in_days, lab_procedure_name, lab_procedure_description, lab_result_date_age_in_days, result_component_name, result_loinc_code, result_value, result_flag
medications: patient_id, visit_id, med_record_id, med_order_date_age_in_days, med_start_date_age_in_days, med_end_date_age_in_days, med_record_type, med_simple_generic_name
problem_list: patient_id, problem_list_id, noted_date_age_in_days, resolved_date_age_in_days, pl_diag
referrals: patient_id, visit_id, referral_id, referral_date_age_in_days, requested_specialty, referral_number_of_visits
```

In emitted rows, `lab_procedure_name`, `lab_procedure_description`, `result_loinc_code`, `result_value`, `med_end_date_age_in_days`, and `resolved_date_age_in_days` remain empty strings. `problem_list` has no visit key; referral, lab, and medication rows use their visible event's source-point visit link.

The closed vocabulary is `UNDERNUTRITION_DIAGNOSIS_CODE` = `SYN-UNDERNUTRITION`, `UNDERNUTRITION_WEIGHT_COMPONENT` = `SYN-UNDERNUTRITION-WEIGHT-EVIDENCE`, `UNDERNUTRITION_HEIGHT_COMPONENT` = `SYN-UNDERNUTRITION-HEIGHT-EVIDENCE`, `UNDERNUTRITION_LAB_RESULT_FLAG` = `Synthetic`, `UNDERNUTRITION_REFERRAL_SPECIALTY` = `Synthetic Pediatric Nutrition`, `UNDERNUTRITION_MEDICATION_NAME` = `Synthetic nutrition-supplement intervention`, and `UNDERNUTRITION_MEDICATION_RECORD_TYPE` = `Internal`; identifiers use the `undernutrition-ancillary-id-v1` namespace. These labels are fictional rather than clinical terminology: they are not ICD, LOINC, or RxNorm terminology or clinical value claims. Lab rows carry the in-memory marker as `result_flag="Synthetic"`, medication rows use `med_record_type="Internal"`, and the fictional nutrition-supplement wording is fixture content, not nutrition guidance or a treatment recommendation.

`UndernutritionModule` owns the weight/BMI-first decline, delayed height effect, and optional partial recovery as hidden upstream state; none becomes a row. A visible `recognition` creates one `referrals` row, a visible `workup` creates two `labs` per workup in one order, and a visible `diagnosis` creates one unresolved `problem_list` row. `UNDERNUTRITION_LAB_COMPONENT_NAMES` fixes the weight component first and height component second. The fixed lab component value order is `SYN-UNDERNUTRITION-WEIGHT-EVIDENCE` and `SYN-UNDERNUTRITION-HEIGHT-EVIDENCE`. The lab result age equals the workup order age plus `result_delay_days`, supplied as `UndernutritionAncillaryPolicy.result_delay_days`. A visible diagnosis plus a private `treatment_start` at or after diagnosis creates one medication linked to the diagnosis source-point visit; treatment is suppressed when diagnosis is absent, censored, or later than treatment. Treatment response/nonresponse creates no row.

The validator statuses are fixed as `PASS`, `FAIL`, and `UNEVALUABLE`, with precedence `FAIL` > `UNEVALUABLE` > `PASS`. `UNDERNUTRITION_ANCILLARY_CHECK_NAMES` fixes this check order: `pathway_scope`, `row_schema`, `causal_timing`, `cross_resource_links`, and `source_evidence`. `UNDERNUTRITION_ANCILLARY_REASON_CODES_BY_STATUS` and `UNDERNUTRITION_ANCILLARY_REASON_CODES` expose the fixed aggregate-only reason vocabulary.

- `PASS`: `OK`
- `FAIL`: `CAUSAL_TIMING_INVALID`, `CROSS_RESOURCE_LINK_INVALID`, `DUPLICATE_ROW`, `EVENT_ORDER_INVALID`, `INVALID_CODE`, `INVALID_ID`, `INVALID_VALUE`, `MALFORMED_PROJECTION`, `PATHWAY_OUT_OF_SCOPE`, `PATHWAY_SCOPE_INVALID`, `PATIENT_MISMATCH`, `ROW_SCHEMA_INVALID`, `SCHEMA_SHAPE_INVALID`, `SOURCE_EVIDENCE_INVALID`, `TIMING_INVALID`, `VISIT_REFERENCE_INVALID`
- `UNEVALUABLE`: `INSUFFICIENT_EVIDENCE`, `MALFORMED_ANCILLARY`, `MALFORMED_MEMBER`, `SOURCE_EVIDENCE_UNAVAILABLE`

For every non-undernutrition member, this direct evaluator-only projection returns four empty tuples, and no `obesity_flag` is emitted or derived. The published `development-all-disorders` route separately composes the matching undernutrition projection, serializes its in-memory `Synthetic` marker to the descriptor sentinel, and exports it within the exact eight-resource package. Runtime integration and package export remain deferred for this direct evaluator-only API and any unreviewed pathway; the all-disorder route is the reviewed package-level exception. That route remains fictional coverage; prevalence calibration, demographic calibration, privacy/non-matchability evaluation, clinical review, nutrition guidance, release authorization, real data, held-out data, dedicated nutrition resources, clinical terminology mappings, and serialization of the in-memory `Synthetic` marker in an unreviewed route remain deferred as separate reviewed contracts and are not ordinary-development prerequisites. No deferred workflow may introduce patient rows or alter this typed projection contract.

Optional Synthea conformance is likewise deferred for the direct undernutrition API and any unreviewed pathway; the published all-disorder route remains the reviewed package-level exception.
...
validator contract. The explicit `development-realistic` route is the first
package-level exception documented above: it serializes the validated fictional
GHD ancillary rows via `export_exact_schema_package` and the same oracle-owned
augmented resources. The published `development-all-disorders` CLI is the second
package-level route and composes only the matching reviewed projections for the
seven supported ancillary targets. The evaluator-only APIs below remain
in-memory seams; they do not themselves write files or expose hidden truth. The
...
The evaluator-only composition and module APIs above do not themselves write diagnosis, laboratory, medication, or referral rows. The published `development-all-disorders` CLI is the second package-level route and composes the matching reviewed projections for growth-hormone deficiency, pediatric hypothyroidism, celiac disease, small-for-gestational-age, Turner syndrome, undernutrition, and excess weight; only matching reviewed projections create ancillary rows. Prevalence and demographic calibration; held-out validation; privacy auditing; package-level counterfactual worlds; clinical approval of a reference; and Synthea conformance remain deferred gates. These scenarios are not clinically validated and do not claim a match to real EHR or growth data. The healthy age-730+ smoke/export boundary remains three visits at ages 730, 1095, and 1460 days, and the existing non-matchability limitation still applies: synthetic generation alone cannot establish that a profile cannot be matched to a real patient.
...
The hypothyroidism, celiac-disease, SGA, Turner-syndrome, undernutrition, and excess-weight modules remain trajectory-only when called through this direct evaluator-only API: their generic workup, diagnosis, treatment, birth-state, or sex-reference events remain hidden evaluator state, and this API writes no visible rows. The published `development-all-disorders` CLI separately consumes the seven matching reviewed projections for package-level fictional coverage, with typed lab markers normalized only at serialization. Any additional disorder-specific descendants or resources require a separate reviewed resource contract.
...
These defaults are uncalibrated development scenarios. `LatentTrajectory.disorder` and `LatentTrajectory.events` are evaluator-only hidden truth and event traces; they are not exported, and visible CSV generation remains unchanged for this direct module layer. The published all-disorder package route separately projects reviewed disorder-critical labs/medications/referrals, but its prevalence, demographic calibration, held-out validation, privacy auditing, and package-level counterfactual worlds remain later gates. No real patient data, clinical claim, or privacy claim is introduced by this layer.
...
That behavior is intentional. Do not treat a command-line failure as a missing flag or bypass the injected-reference/oracle boundary. The four explicit development profiles are documented above; `development-all-disorders` is the second package-level route that composes the seven reviewed ancillary projections, while all four use the pinned source-matched runtime for test-only reproducibility and do not enable a production route. Wire a reviewed production reference and authoritative oracle through an explicit API/CLI design before enabling a production route.
