...
For every non-undernutrition member, the projection returns four empty tuples, and no `obesity_flag` is emitted or derived. Runtime integration, package export, prevalence calibration, demographic calibration, privacy/non-matchability evaluation, clinical review, nutrition guidance, release authorization, real data, held-out data, dedicated nutrition resources, clinical terminology mappings, serialization of the in-memory `Synthetic` marker, and optional Synthea conformance remain deferred as separate reviewed contracts in this ordinary-development fixture contract and are not ordinary-development prerequisites. No deferred workflow may introduce patient rows or alter this typed projection contract.
