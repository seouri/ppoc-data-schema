...
`project_undernutrition_ancillary_resources(member, shape, policy)` and `validate_undernutrition_ancillary_resources(member, projection, policy)` are deterministic, typed in-memory, exact-schema, evaluator-only APIs for one fictional member. The public values are `UndernutritionAncillaryPolicy`, `UndernutritionAncillaryProjection`, `UndernutritionAncillaryProjectionUnavailable`, `UndernutritionAncillaryValidationStatus`, `UndernutritionAncillaryCheck`, and `UndernutritionAncillaryValidationReport`. This boundary accepts a typed `CohortMember`, an already extracted `ResourceShape`, and a policy. The current descriptor has four named resource shapes in fixed order—`labs`, `medications`, `problem_list`, and `referrals`—and projection tuples preserve its descriptor field order and empty-string missing-value conventions.

The closed vocabulary is `UNDERNUTRITION_DIAGNOSIS_CODE` = `SYN-UNDERNUTRITION`, `UNDERNUTRITION_WEIGHT_COMPONENT` = `SYN-UNDERNUTRITION-WEIGHT-EVIDENCE`, `UNDERNUTRITION_HEIGHT_COMPONENT` = `SYN-UNDERNUTRITION-HEIGHT-EVIDENCE`, `UNDERNUTRITION_LAB_RESULT_FLAG` = `Synthetic`, `UNDERNUTRITION_REFERRAL_SPECIALTY` = `Synthetic Pediatric Nutrition`, `UNDERNUTRITION_MEDICATION_NAME` = `Synthetic nutrition-supplement intervention`, and `UNDERNUTRITION_MEDICATION_RECORD_TYPE` = `Internal`; identifiers use the `undernutrition-ancillary-id-v1` namespace. These labels are fictional rather than clinical terminology: they are not ICD, LOINC, or RxNorm terminology or clinical value claims. Lab rows serialize the in-memory marker as `result_flag="Synthetic"`, medication rows use `med_record_type="Internal"`, and unrepresented result codes and values remain empty. The fictional nutrition-supplement wording is fixture content, not nutrition guidance or a treatment recommendation.

`UndernutritionModule` owns the weight/BMI-first decline, delayed height effect, and optional partial recovery as hidden upstream state; none becomes a row. A visible `recognition` creates one `referrals` row, a visible `workup` creates two `labs` per workup in one order, and a visible `diagnosis` creates one unresolved `problem_list` row. Referral and lab rows use each visible event's source-point visit link. A visible diagnosis plus a private `treatment_start` at or after diagnosis creates one medication linked to the diagnosis source-point visit; treatment is suppressed when diagnosis is absent, censored, or later than treatment. Treatment response/nonresponse creates no row.

For every non-undernutrition member, the projection returns four empty tuples, and no `obesity_flag` is emitted or derived. Runtime/package integration, prevalence/demographic calibration, privacy/non-matchability, clinical/nutrition/release claims, real or held-out data, dedicated resource/terminology expansion, and optional Synthea conformance remain deferred in this ordinary-development fixture contract and are not ordinary-development prerequisites.
