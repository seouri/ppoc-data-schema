...
This direct SGA API has no file or CLI integration; the published `development-all-disorders` route separately composes its matching typed projection for package-level fictional coverage. That route is not prevalence/demographic calibration, privacy/non-matchability evidence, clinical review or guidance, release authorization, real or held-out data work, or Synthea conformance. Additional resource expansion, prevalence/demographic calibration, privacy/non-matchability, clinical review, release authorization, real or held-out data, and optional Synthea conformance remain deferred and are not ordinary-development prerequisites.
