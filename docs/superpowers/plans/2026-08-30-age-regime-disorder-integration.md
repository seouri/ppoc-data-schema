...
- [x] Register `DisorderKind.EXCESS_WEIGHT`, its named counterfactual stream, built-in module contract, untreated/treated golden forced-coverage cases, and the age-regime composition path without changing the visible package schema.
- [x] Add deterministic, schedule, overflow, state-kind, treatment, age-regime composition, and golden-pattern tests; update evaluator-only documentation while retaining obesity-specific ancillary pathways as deferred.
