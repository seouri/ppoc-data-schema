...
- [x] Merge to `main`, rerun the full verification suite on merged `main`, push, and verify `HEAD == origin/main`. The privacy review worktree and ignored SDD evidence are intentionally preserved for audit traceability; unrelated worktrees and pre-existing caches remain untouched.

Merged to `main` at `da6b2f4`; post-merge verification passed with 1318 tests, Ruff, schema, and diff checks. Final parity and push are recorded in the SDD ledger; the resulting metadata tip is `dcfadfc`.
