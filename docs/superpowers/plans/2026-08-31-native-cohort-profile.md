...
Store aggregate weights in canonical registry order. Validate finite nonnegative values and a total within `0.99..1.01`, then normalize only the in-memory categorical weights. Preserve the recorded target rates separately as evidence fields; never turn them into module probabilities. Map explicitly sampled blank ethnicity/race values to visible `Unknown` only in a private projection helper, use the empty-string missing sentinel for an unselected race slot two, and use the same sentinel for slots three through eight because no higher-order target is modeled. Keep `to_mapping()` aggregate-only and omit supports, denominators, source metadata beyond the safe artifact identity, and any patient-attributable material.
