...
- Boundary: the raw CLI remains a development candidate; the explicit `development-smoke`/`development-cohort` profiles may use the test-only adapter for wholly synthetic packages, while the default/no-profile and production command remain fail-closed and no authority is conferred.
