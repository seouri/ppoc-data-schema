...
The current native development CLI makes the development profile explicit: `development-smoke` is the compact three-visit route, `development-cohort` is the fixed healthy/GHD coverage route, and `development-realistic` is the target-shaped route. The latter freezes aggregate demographic weights and the `growth_dx_flag`-derived healthy/GHD prior from the checked-in `schema/stats.json` snapshot, uses the pinned CDC trajectories, places only a fixed synthetic `E23.0` token at sampled GHD diagnosis visits so the source augmenter exposes the generated growth-disorder flag, and promotes the typed fictional GHD ancillary projection (referral, two labs, problem row, and causally eligible medication) through the existing exact-schema exporter. In-memory lab markers are serialized as the descriptor's empty-string sentinel; the generic empty-ancillary and paired-world paths remain unchanged. All three routes remain wholly generated, exact-schema, and test-only; the snapshot shape is a development scenario input rather than a clinical or population-validity claim.
