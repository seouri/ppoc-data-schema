...
The current ordinary-development implementation exposes pediatric hypothyroidism, celiac disease, small-for-gestational-age (SGA), Turner syndrome, undernutrition, and excess weight as uncalibrated, evaluator-only trajectory modules with checked-in golden cases; none is enabled in the visible cohort/package route and none has disorder-specific ancillary projection (for example, TSH/free-T4/levothyroxine for hypothyroidism, celiac testing/referral/treatment resources, gestational-age/prematurity resources for SGA, karyotype/estrogen resources for Turner syndrome, nutrition-supplement resources for undernutrition, or obesity-treatment resources for excess weight). Those visible descendants remain separate follow-on contracts.
...
The historical three-profile native development CLI scope remains explicit: `development-smoke` is the compact three-visit route, `development-cohort` is the fixed healthy/GHD coverage route, and `development-realistic` is the target-shaped route. The latter freezes aggregate demographic weights and the `growth_dx_flag`-derived healthy/GHD prior from the checked-in `schema/stats.json` snapshot, uses the pinned CDC trajectories, places only a fixed synthetic `E23.0` token at sampled GHD diagnosis visits so the source augmenter exposes the generated growth-disorder flag, and promotes the typed fictional GHD ancillary projection (referral, two labs, problem row, and causally eligible medication) through the existing exact-schema exporter. In-memory lab markers are serialized as the descriptor's empty-string sentinel; the generic empty-ancillary and paired-world paths remain unchanged. A separate `development-all-disorders` follow-on route exercises all ten trajectory modules and its seven reviewed ancillary dispatch targets with a fictional conditional coverage prior; it preserves the exact-schema, hidden-truth, and test-only boundaries. All four routes remain wholly generated and test-only; the snapshot shape is a development scenario input rather than a clinical or population-validity claim.
