...
**Date:** 2026-08-31
**Status:** Implementation complete; evaluator/development-only; fixed fictional profile does not require calibration; prevalence, clinical, privacy, and release gates pending
**Parent:** [Synthetic Pediatric Growth Fixture System Design](2026-08-30-synthetic-growth-fixtures-design.md)
**Prerequisites:** [Age-regime disorder integration](2026-08-30-age-regime-disorder-integration-design.md), the optional [governed calibration core](2026-08-31-governed-calibration-core-design.md), [evaluator observation frame](2026-08-31-observation-frame-design.md), and [observed resource contract](2026-08-31-observed-resource-contract-design.md)

## Purpose

The repository now has deterministic healthy and growth-disorder trajectory kernels, an evaluator-only observation frame, descriptor-shaped observed resource bundles, an exact-schema package bridge, and both a fixed fictional development profile and an optional governed aggregate calibration artifact. It does not yet have the native cohort layer that composes those pieces into a reproducible healthy-plus-disorder development population. This slice adds that missing in-memory orchestration boundary.

The cohort profile samples fictional demographics from either the fixed development profile or explicitly released aggregate calibration cells, chooses latent modules from an explicit clinical prior, generates age-regime trajectories, projects them through the observation process, and optionally produces validated observed-resource bundles. It keeps latent module truth and trajectory state in evaluator-only objects and exposes only visible frame/bundle mappings through ordinary serialization.

This slice does not turn recorded diagnosis prevalence into latent disease prevalence, assign labels to force a final observed rate, fit parameters from patient rows, or claim that the resulting population is representative. When an artifact-backed profile is selected, the calibration artifact supplies aggregate demographic and recorded-outcome targets; for ordinary development, fixed fictional weights supply those inputs. Explicit module priors and observation policies remain versioned caller inputs. Held-out fidelity evaluation is the later optional gate that determines whether artifact-backed choices reproduce approved targets.
...
2. Accept a `CalibrationSamplingProfile` built from the fixed fictional development configuration or from an already-loaded `CalibrationArtifact`; never accept a real-data path, partition key, patient row, sequence, held-out report, privacy report, or hidden evaluator truth as a cohort input.
3. When an artifact-backed profile is selected, require complete released demographic target cells. A suppressed or missing target fails closed; it is never silently dropped, normalized to zero, or replaced by a hidden real-data fallback.
4. Sample recorded sex, ethnicity, primary race, and the selected profile's race-multiselect indicator from aggregate weights, preserving an explicit mapping for blank/nonresponse cells into the visible fictional category vocabulary.
...
7. Guarantee deterministic patient identifiers, demographic draws, module choices, trajectories, observations, and bundle ordering for equal profile, seed, reference, policy, profile inputs, and module configuration.
8. Fail closed on malformed artifact-backed calibration inputs, incompatible schema/registry metadata, invalid module priors, unsupported reference-sex mappings, invalid observation results, duplicate identifiers, and any hidden-truth or filesystem boundary attempt.
...
- The cohort API remains in-memory and does not call the exporter. The explicit development CLI supplies the checked-in test-only growth reference/oracle; the default/no-profile command remains fail closed and production authority is still absent.
...
- No prevalence-forcing allocator, iterative fitting loop, or optimizer is included. Recorded diagnosis/flag targets are checked later by optional governed held-out validation when a population-fidelity claim is requested.
...
`CalibrationSamplingProfile` is the common in-memory representation for both the fixed fictional development profile and an artifact-derived profile. The latter contains aggregate values only: its mapping and representation omit source paths, partition keys, patient/visit identifiers, target supports/denominators, hidden labels, and attack output. `recorded_healthy_probability` and `recorded_growth_dx_probability` are evidence for later validation and are not read by module selection.
...
The profile samples demographics from aggregate weights rather than copying or resampling patient records. In artifact-backed mode, a blank ethnicity/race cell is an observed nonresponse category in the calibration artifact; an explicitly sampled blank category maps to visible `Unknown` and records that lossy mapping in the profile contract. Race slot one is sampled from the selected profile's primary-race distribution. When the race-multiselect draw succeeds, slot two is sampled from that same profile distribution; when it does not, slot two is the empty-string missing sentinel. Slots three through eight are also empty-string missing sentinels because no higher-order aggregate target is modeled. This preserves the source-shaped mostly blank secondary-race fields and is an explicit approximation, not evidence that a source population's full multiselect structure was reproduced.

The latent prior is the only source of module prevalence in this slice. It may be a fixed fictional development setting or a separately approved clinical configuration, but the API does not infer it from `growth_dx_flag`, `healthy_flag`, or any other recorded outcome. A healthy module can have no recorded diagnosis event; a disorder module can remain unrecognized under the observation policy. The resulting observed diagnosis and flag rates are measured by optional later held-out validation rather than forced by a final allocation pass.
...
The descriptor is an already-loaded mapping and is used only to extract the existing six-resource shape. The cohort module never reads a descriptor path or CSV, imports governed calibration-input/held-out/privacy modules, calls a package writer, or accepts a real-data argument. The current explicit development CLI constructs this API with the fixed fictional profile, checked-in public reference, and test-only derivation oracle; an artifact-backed or future production CLI must add only the applicable reference, terminology, derivation, validation, and review gates for the claims it makes.
...
5. The explicit development CLI, exact-schema lifecycle, and test-only augmentation remain available without governed inputs; authoritative augmentation, held-out validation, privacy audit, clinical approval, and the Synthea route remain separate and explicitly deferred claim gates.
