...
The profile samples demographics from aggregate weights rather than copying or resampling patient records. In artifact-backed mode, a blank ethnicity/race cell is an observed nonresponse category in the calibration artifact; in either mode, the visible resource vocabulary has no blank category, so the fixed projection maps it to `Unknown` and records that lossy mapping in the profile contract. Race slot one is sampled from the selected profile's primary-race distribution. When the race-multiselect draw succeeds, slot two is sampled from that same profile distribution; slots three through eight remain `Unknown` because no higher-order aggregate target is modeled. This is an explicit approximation, not evidence that a source population's full multiselect structure was reproduced.
