...
5. FHIR R4 parsing for `Patient`, `Encounter`, `Observation`, and `Condition` resources. The parser emits descriptor-shaped `patients`, `visits`, and empty ancillary base rows; only syntactically valid ICD-10 coding-system entries are placed in the nearest eligible visit, without preserving Synthea identifiers.
...
`scripts/synthea/overlay/modules/ppoc_growth_disorder.json` is the only module accepted by this slice. Its transition graph is deliberately small: fixed prior, one delay, condition onset, ambulatory evaluation encounter, two fictional laboratory observations, encounter end, and terminal state. The module uses the valid ICD-10-CM code `E23.0` as a completely synthetic development diagnosis; its occurrence is not a clinical or prevalence claim. The parser recognizes the code only in output produced by this pinned overlay.
...
For each patient, FHIR `gender` maps to `F/M/U`; US Core race and ethnicity extension text maps to the descriptor enum, with absent or unsupported values projected to `Unknown`. The parser contract is versioned as `synthea-fhir-r4-parser-v2`: up to eight race slots are populated, and all remaining slots use the descriptor's empty-string missing sentinel. When no race extension is present, slot one is `Unknown` and slots two through eight are empty. Patient IDs are `synthetic_id(seed, "synthea-patient", sorted_index)` and visit IDs are `synthetic_id(seed, "synthea-visit", stable_patient_index * 100000 + visit_index)`. Synthea IDs and generated names are lookup-only and never leave the parser.
...
Conditions are filtered to ICD-10 coding-system entries and normalized to their code text; syntactically invalid or non-ICD codings are discarded. A retained condition is associated with the referenced encounter when present. Otherwise, it is assigned to the first anthropometric visit on or after its onset date, or the final eligible visit when the onset is after observation end. At most 33 diagnosis slots are populated in stable code order. Ancillary resources remain empty in this slice; the existing augmenter derives both augmented resources from the base rows.

The report's `healthy_count` is the non-GHD trajectory group: it means no fictional `E23.0` event or growth attenuation was applied, not that unrelated synthetic Synthea conditions were removed. Unrelated conditions are retained only when they carry a valid ICD-10 coding-system entry; therefore populated `enc_diag_*` slots in the package contain ICD-10 codes only. Synthea labs, medications, problem-list entries, and referrals remain empty in this first adapter slice.
