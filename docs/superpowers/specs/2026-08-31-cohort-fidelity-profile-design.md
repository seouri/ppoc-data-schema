...
The report also includes fixed observation coverage checks: cohort size, members with at least one visit, and members with at least one recorded event. These are counts and fractions only. Missing frames, mismatched patient identities, duplicate member IDs, nonmonotone ages, or malformed evaluator objects produce `FAIL` without including offending values in the public exception or report. A valid cohort generated by `generate_native_cohort` should pass these structural checks; direct hand-built malformed inputs are intentionally detectable.
