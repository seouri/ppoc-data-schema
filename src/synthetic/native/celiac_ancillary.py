"""Evaluator-only ancillary projection for a fictional celiac pathway.
...
CELIAC_DIAGNOSIS_CODE = "SYN-CELIAC-DISEASE"
CELIAC_TTG_IGA_COMPONENT = "SYN-CELIAC-TTG-IGA"
CELIAC_TOTAL_IGA_COMPONENT = "SYN-CELIAC-TOTAL-IGA"
CELIAC_LAB_COMPONENT_NAMES = (
    CELIAC_TTG_IGA_COMPONENT,
    CELIAC_TOTAL_IGA_COMPONENT,
)
CELIAC_LAB_RESULT_FLAG = "Synthetic"
CELIAC_REFERRAL_SPECIALTY = "Synthetic Pediatric Gastroenterology"
CELIAC_MEDICATION_NAME = "Synthetic gluten-free intervention"
CELIAC_MEDICATION_RECORD_TYPE = "Internal"

# Readable aliases keep the closed fictional vocabulary discoverable without
# accepting caller-supplied terminology.
SYNTHETIC_CELIAC_DIAGNOSIS_CODE = CELIAC_DIAGNOSIS_CODE
SYNTHETIC_CELIAC_TTG_IGA_COMPONENT = CELIAC_TTG_IGA_COMPONENT
SYNTHETIC_CELIAC_TOTAL_IGA_COMPONENT = CELIAC_TOTAL_IGA_COMPONENT
...
class CeliacAncillaryProjectionUnavailable(ValueError):
...
class CeliacAncillaryPolicy:
    ...
    def __repr__(self) -> str:
        return "CeliacAncillaryPolicy(<aggregate-only>)"
...
@dataclass(frozen=True, repr=False)
class CeliacAncillaryProjection:
    ...
    PROJECTION_VERSION: ClassVar[str] = "celiac-ancillary-projection-v1"
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(self.rows) != CELIAC_ANCILLARY_RESOURCE_NAMES:
            raise ValueError(
    ...
    def __repr__(self) -> str:
        return "CeliacAncillaryProjection(<evaluator-only>)"
...
def project_celiac_ancillary_resources(
    member: CohortMember,
    shape: ResourceShape,
    policy: CeliacAncillaryPolicy,
    ...
    if (
        ...
        or not isinstance(policy, CeliacAncillaryPolicy)
    ):
        raise CeliacAncillaryProjectionUnavailable(
            "celiac ancillary projection unavailable"
        )
    ...
    try:
        ...
        if trajectory.disorder.kind is not DisorderKind.CELIAC_DISEASE:
            return CeliacAncillaryProjection(patient_id, shape, rows)

        if any(
            not _REQUIRED_FIELDS[name].issubset(shape.field_names(name))
            for name in CELIAC_ANCILLARY_RESOURCE_NAMES
        ):
            ...
            rows["referrals"] = (
                _resource_row(
                    "referrals",
                    {
                        ...
                        "requested_specialty": CELIAC_REFERRAL_SPECIALTY,
            ...
            rows["labs"] = tuple(
                _resource_row(
                    "labs",
                    {
                        ...
                        "result_loinc_code": "",
                        "result_value": "",
                        "result_flag": CELIAC_LAB_RESULT_FLAG,
                    },
                    shape,
                )
                ...
                rows["medications"] = (
                    _resource_row(
                        "medications",
                        {
                            ...
                            "med_end_date_age_in_days": "",
                            "med_record_type": CELIAC_MEDICATION_RECORD_TYPE,
                            "med_simple_generic_name": CELIAC_MEDICATION_NAME,
                        },
                        shape,
                    ),
                )
        ...
        raise CeliacAncillaryProjectionUnavailable(
            "celiac ancillary projection failed"
        ) from None
...
class CeliacAncillaryValidationStatus(str, Enum):
...
CELIAC_ANCILLARY_REASON_CODES_BY_STATUS: Mapping[
    CeliacAncillaryValidationStatus, frozenset[str]
] = MappingProxyType(
    {
        CeliacAncillaryValidationStatus.PASS: frozenset({"OK"}),
        CeliacAncillaryValidationStatus.FAIL: frozenset(
            {
                "PATIENT_MISMATCH",
        ...
        CeliacAncillaryValidationStatus.UNEVALUABLE: frozenset(
            {
                "MALFORMED_ANCILLARY",
                "MALFORMED_MEMBER",
...
CELIAC_ANCILLARY_REASON_CODES = frozenset(
    reason
    for reasons in CELIAC_ANCILLARY_REASON_CODES_BY_STATUS.values()
    for reason in reasons
)

_CELIAC_INTEGER_FIELDS = frozenset(
    {
        "result_line_num",
        "lab_order_date_age_in_days",
        "lab_result_date_age_in_days",
...
_CELIAC_OPTIONAL_INTEGER_FIELDS = frozenset(
    {"med_end_date_age_in_days", "resolved_date_age_in_days"}
)
_CELIAC_AGE_FIELDS = frozenset(
    field_name
    for field_name in _CELIAC_INTEGER_FIELDS
    if field_name not in {"result_line_num", "referral_number_of_visits"}
)


def _validation_status_for_checks(
    checks: tuple[CeliacAncillaryCheck, ...],
) -> CeliacAncillaryValidationStatus:
    if any(
        check.status is CeliacAncillaryValidationStatus.FAIL
        for check in checks
    ):
        return CeliacAncillaryValidationStatus.FAIL
    if any(
        check.status is CeliacAncillaryValidationStatus.UNEVALUABLE
        for check in checks
    ):
        return CeliacAncillaryValidationStatus.UNEVALUABLE
    return CeliacAncillaryValidationStatus.PASS


@dataclass(frozen=True, repr=False)
class CeliacAncillaryCheck:
    """One fixed aggregate-only validation check."""

    name: str
    status: CeliacAncillaryValidationStatus
    reason_code: str
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.name, str) or self.name not in self.CHECK_NAMES:
            raise ValueError("unknown celiac ancillary check name")
        if not isinstance(
            self.status, CeliacAncillaryValidationStatus
        ):
            raise TypeError(
                "status must be a CeliacAncillaryValidationStatus"
            )
        if (
            not isinstance(self.reason_code, str)
            or self.reason_code
            not in CELIAC_ANCILLARY_REASON_CODES_BY_STATUS[
                self.status
            ]
        ):
...
@dataclass(frozen=True, repr=False)
class CeliacAncillaryValidationReport:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            isinstance(check, CeliacAncillaryCheck)
            for check in self.checks
        ):
            raise TypeError(
                "checks must contain CeliacAncillaryCheck values"
            )
        names = tuple(check.name for check in self.checks)
        if len(names) != len(set(names)) or set(names) != set(self.CHECK_NAMES):
            raise ValueError(
                "checks must contain every fixed celiac ancillary "
                "check exactly once"
            )
        ordered = tuple(
        ...
        if self.status is not _validation_status_for_checks(ordered):
            raise ValueError("status must match celiac check statuses")
        ...
        return MappingProxyType(
            {
                ...
                for status in CeliacAncillaryValidationStatus
            }
        )
...
def _ancillary_row_types_are_valid(values: Mapping[str, object]) -> bool:
    ...
    for field_name, value in values.items():
        if field_name in _CELIAC_INTEGER_FIELDS:
            if value == "":
                if field_name not in _CELIAC_OPTIONAL_INTEGER_FIELDS:
                    return False
            elif (
...
def _source_independent_row_reason(
    ...
    for field_name, expected_value in expected.items():
        if field_name in _CELIAC_INTEGER_FIELDS or field_name in {
            "visit_id",
            "result_component_name",
            "patient_id",
        }:
    ...
    for field_name in _CELIAC_OPTIONAL_INTEGER_FIELDS:
...
def _celiac_ancillary_report(
    ...
    checks = tuple(
        ...
        for name in CELIAC_ANCILLARY_CHECK_NAMES
    )
    return CeliacAncillaryValidationReport(
        _validation_status_for_checks(checks), checks
    )


def validate_celiac_ancillary_resources(
    member: CohortMember,
    ...
    if (
        not isinstance(member, CohortMember)
        or not isinstance(projection, CeliacAncillaryProjection)
        or not isinstance(policy, CeliacAncillaryPolicy)
    ):
        raise CeliacAncillaryProjectionUnavailable(
            "celiac ancillary projection unavailable"
    ...
    states: dict[
        str, tuple[CeliacAncillaryValidationStatus, str]
    ] = {
        name: (CeliacAncillaryValidationStatus.PASS, "OK")
        for name in CELIAC_ANCILLARY_CHECK_NAMES
    }
    ...
    def mark(
        ...
        if current is CeliacAncillaryValidationStatus.FAIL:
            return
        if (
            status is CeliacAncillaryValidationStatus.FAIL
            or current is CeliacAncillaryValidationStatus.PASS
        ):
    ...
    try:
        ...
        if not isinstance(trajectory, AgeRegimeDisorderTrajectory):
            ...
            target_kind = (
                ...
                is DisorderKind.CELIAC_DISEASE
            )
            if target_kind:
        ...
        if not isinstance(rows, Mapping):
            ...
            rows = {}
        elif tuple(rows) != CELIAC_ANCILLARY_RESOURCE_NAMES:
            mark(
                "row_schema",
                CeliacAncillaryValidationStatus.FAIL,
                "SCHEMA_SHAPE_INVALID",
        ...
        for resource_name in CELIAC_ANCILLARY_RESOURCE_NAMES:
            ...
            for row in resource_rows:
                ...
                if types_valid:
                    for field_name in _CELIAC_AGE_FIELDS:
            ...
            if resource_name == "labs" and resource_rows:
                if lab_pairs != {
                    (1, CELIAC_TTG_IGA_COMPONENT),
                    (2, CELIAC_TOTAL_IGA_COMPONENT),
    ...
    try:
        ...
        return _celiac_ancillary_report(states)
    ...
    expected: CeliacAncillaryProjection | None = None
    if isinstance(projection.shape, ResourceShape):
        try:
            expected = project_celiac_ancillary_resources(
    ...
    if expected is not None and not any(
        states[name][0] is CeliacAncillaryValidationStatus.FAIL
        for name in (
            "pathway_scope",
            "row_schema",
            "causal_timing",
