...
def _policy_ancillary(**changes: object) -> PediatricHypothyroidismAncillaryPolicy:
...
def _member(
    *,
    ...
    diagnosed: bool = True,
    kind: DisorderKind = DisorderKind.PEDIATRIC_HYPOTHYROIDISM,
...
def _resources(member: CohortMember, **policy_changes: object) -> dict[str, object]:
    return project_pediatric_hypothyroidism_ancillary_resources(
        member,
        _shape(),
...
def test_diagnosed_hypothyroidism_projects_exact_four_resource_contract() -> None:
    ...
    assert tuple(resources) == PEDIATRIC_HYPOTHYROIDISM_ANCILLARY_RESOURCE_NAMES  # type: ignore[arg-type]
    ...
    assert referral["requested_specialty"] == PEDIATRIC_HYPOTHYROIDISM_REFERRAL_SPECIALTY
    ...
    assert all(row["result_flag"] == PEDIATRIC_HYPOTHYROIDISM_LAB_RESULT_FLAG for row in labs)
    ...
    assert problem["pl_diag"] == PEDIATRIC_HYPOTHYROIDISM_DIAGNOSIS_CODE
    ...
    assert medication["med_record_type"] == PEDIATRIC_HYPOTHYROIDISM_MEDICATION_RECORD_TYPE
    assert medication["med_simple_generic_name"] == PEDIATRIC_HYPOTHYROIDISM_MEDICATION_NAME
...
def test_projection_handles_visible_event_combinations_and_optional_treatment() -> None:
    ...
    assert all(not unrecognized[name] for name in PEDIATRIC_HYPOTHYROIDISM_ANCILLARY_RESOURCE_NAMES)
...
def test_projection_replays_deterministically_without_mutating_member() -> None:
    ...
    first = project_pediatric_hypothyroidism_ancillary_resources(
        member, _shape(), _policy_ancillary()
    )
    second = project_pediatric_hypothyroidism_ancillary_resources(
        member, _shape(), _policy_ancillary()
    )
...
def test_projection_uses_fixed_redacted_errors_for_wrong_types_and_tampered_truth() -> None:
    with pytest.raises(
        ...
        project_pediatric_hypothyroidism_ancillary_resources(  # type: ignore[arg-type]
