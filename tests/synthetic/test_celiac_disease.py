...
from synthetic.native.clinical_modules import CeliacDiseaseConfig, CeliacDiseaseModule
from synthetic.randomness import NamedRandomStreams
from tests.synthetic.fakes import RegimeLinearTestReference

PATIENT = PatientState("syn-patient-celiac", "F", "F")


def test_celiac_disease_samples_reproducibly_from_its_scoped_stream() -> None:
    module = CeliacDiseaseModule()
    first = module.sample_state(PATIENT, NamedRandomStreams(20260902, 4))
    second = module.sample_state(PATIENT, NamedRandomStreams(20260902, 4))

    assert first == second
    assert first.kind is DisorderKind.CELIAC_DISEASE
    assert module.config.onset_min_age_days <= first.onset_age_days <= module.config.onset_max_age_days  # type: ignore[operator]


def test_celiac_weight_decline_precedes_height_decline_without_treatment() -> None:
    config = CeliacDiseaseConfig(
    ...
    module = CeliacDiseaseModule(config)
    ...
    assert module.bmi_z_delta(state, onset + config.height_onset_delay_days) < 0  # type: ignore[operator]
    assert module.height_z_delta(
        state, onset + config.height_onset_delay_days + 1
...
def test_treated_celiac_preserves_pre_treatment_effects_and_partially_recovers() -> None:
    config = CeliacDiseaseConfig(
        onset_min_age_days=1000,
        onset_max_age_days=1000,
        severity_min=1.0,
...
@pytest.mark.parametrize(
    "factory",
    [
        lambda: CeliacDiseaseConfig(onset_min_age_days=10**1000),
        lambda: CeliacDiseaseConfig(bmi_progression_days=10**1000),
        lambda: CeliacDiseaseConfig(severity_min=math.nan),
        lambda: CeliacDiseaseConfig(treatment_probability=1.1),
        lambda: CeliacDiseaseConfig(
            treatment_response_min=0.9, treatment_response_max=0.8
        ),
        lambda: CeliacDiseaseConfig(onset_min_age_days=4000, onset_max_age_days=3000),
    ],
)
def test_celiac_config_rejects_invalid_values(factory: object) -> None:
    with pytest.raises(ValueError):
        factory()


def test_celiac_config_is_frozen_and_versioned() -> None:
    config = CeliacDiseaseConfig()
    assert config.module_version == "celiac-disease-v1"
    with pytest.raises(FrozenInstanceError):
        config.bmi_progression_days = 30  # type: ignore[misc]


def test_celiac_constructor_rejects_config_version_drift() -> None:
    config = CeliacDiseaseConfig()
    object.__setattr__(config, "module_version", "celiac-disease-v999")
    with pytest.raises(ValueError, match="version"):
        CeliacDiseaseModule(config)


def test_age_regime_composition_preserves_celiac_weight_identity() -> None:
    config = CeliacDiseaseConfig(
        onset_min_age_days=1000,
        onset_max_age_days=1000,
        severity_min=1.0,
    ...
    result = AgeRegimeDisorderKernel(physiology, module).generate(
        PATIENT,
        (0, 730, 1000, 1180, 1450, 1815, 3000, 7305),
    ...
    assert result.physiology.points[3].height_z == pytest.approx(
        result.physiology.points[2].height_z
    )
