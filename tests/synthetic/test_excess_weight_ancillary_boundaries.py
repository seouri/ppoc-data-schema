...
def test_module_has_only_native_safe_imports_and_no_lifecycle_calls() -> None:
    source = Path(excess_weight_ancillary.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source)
    calls = {
        node.func.id.lower()
        for node in ast.walk(tree)
...
def test_public_interfaces_have_narrow_typed_in_memory_signatures() -> None:
    assert tuple(
        inspect.signature(
            excess_weight_ancillary.project_excess_weight_ancillary_resources
        ).parameters
    ) == ("member", "shape", "policy")
    assert tuple(
        inspect.signature(
            excess_weight_ancillary.validate_excess_weight_ancillary_resources
        ).parameters
