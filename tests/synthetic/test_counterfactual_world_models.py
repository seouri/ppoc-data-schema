...
def _checks(
    ...
    return tuple(
        CounterfactualWorldCheck(
            name,
            status,
            FAILURE_REASONS[name] if status is CounterfactualWorldValidationStatus.FAIL else reason,
        )
...
def test_world_report_is_immutable_canonical_and_aggregate_only() -> None:
    ...
    checks[0] = CounterfactualWorldCheck(
        "pair_binding", CounterfactualWorldValidationStatus.FAIL, "PAIR_BINDING_INVALID"
    )
    ...
    assert set(FAILURE_REASONS.values()) < COUNTERFACTUAL_WORLD_REASON_CODES
...
def test_world_report_rejects_incomplete_or_status_incompatible_checks() -> None:
    ...
    with pytest.raises(ValueError, match="reason"):
        CounterfactualWorldCheck(
            "pair_binding",
            CounterfactualWorldValidationStatus.FAIL,
