...
def test_module_has_only_native_safe_imports_and_no_lifecycle_calls() -> None:
    source = Path(celiac_ancillary.__file__).read_text(
        encoding="utf-8"
    )
    tree = ast.parse(source)
    calls = {
...
def test_public_interfaces_have_narrow_typed_in_memory_signatures() -> None:
    assert tuple(
        inspect.signature(
            celiac_ancillary.project_celiac_ancillary_resources
        ).parameters
    ) == ("member", "shape", "policy")
    assert tuple(
        inspect.signature(
            celiac_ancillary.validate_celiac_ancillary_resources
        ).parameters
