...
| **Observation and care process are informative** | 3.2, 3.7, 5.5, 5.14 — visits per year separates the growth-diagnosis label at 0.664 on the rank statistic, and the count of head-circumference measurements at 0.605 against 0.586 for the stunting flag | Utilization is a measured shortcut here rather than a suspected one: how often a child was measured carries more about that label than whether the measurement was low. E2 describes the care-process signal, E5 manipulates schedule while holding physiology fixed, E7 crosses the two; the referral layer requires a matched index date and look-forward. |
| **Candidate labels are not trajectory truth** | 5.6, 5.7 — `growth_dx_flag` covers 35,907 patients at median age 0.027 years, and the tracked-code panel is dominated by perinatal codes; 5.13 — `visits_count_pre_dx` recovers the flag at 99.7% recall | Diagnosis and `healthy_flag` are not the primary reference standard. Layer C uses referral as a secondary positive-unlabelled action outcome; the counterfactual core uses constructed truth or within-subject response changes. |
...
- **A feature set assembled from the delivered columns without a leakage screen.** The raw diagnosis slots reconstruct the tracked panel exactly, one derived column is the flag itself, and the medication records leak through products no growth-oriented exclusion list would name (5.13); columns describing the shape of a record separate the label about as well as the growth channels do (5.14).
