...
The data report also carries analyses this file never had: the cohort construction funnel (1.4), the checklist coverage map naming nine checks this extract cannot support (2.1), the raw-versus-augmented layer comparison (1.3), a field index over all 254 columns (6.1), and a shortcut audit screening every field and every derived column against the growth-diagnosis label (5.14, 5.15). `reports/audit_coverage.py` checks that every analysis once here still has a counterpart there.
...
| --- | --- | --- |
| **Longitudinal, irregular, repeated trajectories** | 4.1, 4.10 — lag-1 height-z autocorrelation 0.925, intraclass correlation 0.822 | E5a changes schedule density while preserving deviation-carrying visits, matched noise, and measurement availability; uncertainty is estimated at the patient or trajectory level, since a child contributes about 1.2 independent observations however many visits are recorded. |
| **Observation and care process are informative** | 3.2, 3.7, 5.5, 5.15 — visits per year separates the growth-diagnosis label at 0.664 on the rank statistic, and the count of head-circumference measurements at 0.605 against 0.586 for the stunting flag | Utilization is a measured shortcut here rather than a suspected one: how often a child was measured carries more about that label than whether the measurement was low. E2 describes the care-process signal, E5 manipulates schedule while holding physiology fixed, E7 crosses the two; the referral layer requires a matched index date and look-forward. |
| **Candidate labels are not trajectory truth** | 5.6, 5.7 — `growth_dx_flag` covers 35,907 patients at median age 0.027 years, and the tracked-code panel is dominated by perinatal codes; 5.14 — `visits_count_pre_dx` recovers the flag at 99.7% recall | Diagnosis and `healthy_flag` are not the primary reference standard. Layer C uses referral as a secondary positive-unlabelled action outcome; the counterfactual core uses constructed truth or within-subject response changes. |
...
| **Derived longitudinal fields carry a definition the field names do not** | 4.8 — the height delta reproduces on 99.988% of rows under its own interval rule and 43.7% under a naive lag; weight follows a different rule, and the two published columns regenerate only 72.3% of the velocity | Velocity is consumed as distributed and the whole definition travels with it — the channel's interval rule, the 365-day year, and the fact that it divides the unrounded difference rather than the published delta; no velocity here is compared with a visit-to-visit rate. |
...
- **A feature set assembled from the delivered columns without a leakage screen.** The raw diagnosis slots reconstruct the tracked panel exactly, one derived column is the flag itself, and the medication records leak through products no growth-oriented exclusion list would name (5.14); columns describing the shape of a record separate the label about as well as the growth channels do (5.15).
...
8. **Leakage.** Screen every field and derived column against whatever label an analysis uses, and screen again after any change of label or index date — the shortcut set belongs to the label rather than to the extract (5.14, 5.15). Exclude the raw diagnosis slots, `visits_count_pre_dx`, and the treatment and workup records; where a column describes the record rather than the child, make the observation window part of the design instead of dropping the column silently.
